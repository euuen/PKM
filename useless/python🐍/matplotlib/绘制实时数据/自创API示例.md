...
class PID:
    def __init__(self):
        ...
        self.time = 0 # s(秒)为单位
        self.curr_v = 0
        self.target_v = 25 # m/s(米)为单位
        self.mass = 20 # 单位为kg
        self.F = 0 # 单位为N
        self.f = 3 # 单位为N
        
        self.kp = 100 # 比例系数 - 增加响应速度
        self.ki = 1.0  # 积分系数 - 消除稳态误差
        self.kd = 1.0  # 微分系数 - 抑制超调
        ...
        max_points = 200
        self.myfig.add_RTline(lambda x : self.target_v, "b-", update_x_data_func=lambda frame : frame/50, max_points=max_points)
        self.myfig.add_RTline(lambda x : self.curr_v, "r-", update_x_data_func=lambda frame : frame/50, max_points=max_points)
        self.myfig.add_RTline(lambda x : self.F, "g-", update_x_data_func=lambda frame : frame/50, max_points=max_points)
    ...
    def update_data(self):
        ...
        # 输出限幅（假设最大控制力为50N）
        self.F = max(min(self.F, 50), -50)
        
        # 更新误差记录
        self.prev_error = error
        
        # 物理仿真（加入随机干扰） 加入空气阻力
        net_force = self.F - self.f - self.curr_v*0.1  + random.random() * 3 - random.random() * 3
    ...
    def on_key_pressed(self, event):
        ...
        if event.key == 'up':
            ...
            print("左箭头被按下 - 减小PID参数")
            self.kp = max(0.1, self.kp - 0.5)
        elif event.key == 'right':
            print("右箭头被按下 - 增加PID参数")
            self.kp += 0.5
        elif event.key == 'r':
            print("R键被按下 - 重置")
            self.curr_v = 0
        ...
        print(f"当前目标速度: {self.target_v}m/s, kp: {self.kp}")
